...
nlp = spacy.load("en_core_web_sm")  # English tokenizer, tagger, parser, NER, and word vectors
...
def process_text(text):
    ...
    tfidf_vectorized_text = tfidf_vectorizer.fit_transform([" ".join(lemmatized_tokens)])
...
processed_reviews = df['Review'].apply(process_text)

# get list of reviews from dataset
processed_review = processed_reviews.iloc

for index in range(1):
    for key, value in processed_review[index].items():
        print(f"{key}: {value}\n")
    print('#'*80)
